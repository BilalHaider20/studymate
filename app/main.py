...
import io
from app.rag import index_pdf, clear_collection, get_all_chroma_data
...
from app.database import (
    ...
    clear_messages, get_all_documents
)
...
@app.get("/database")
def view_database(request: Request):
    messages = get_all_messages()
    documents = get_all_documents()
    chroma_chunks = get_all_chroma_data()
    return templates.TemplateResponse(
        request=request,
        name="database_viewer.html",
        context={
            "messages": messages,
            "documents": documents,
